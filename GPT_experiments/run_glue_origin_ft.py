...
class ModelArguments:
    ...
    input_seed: Optional[int] = field(
        default=None, metadata={"help": "input the random seed."}
    )
    
    """
    Add fine-tuning config here
    """
    ft_method: Optional[str] = field(
        default=None, metadata={"help": "The fine-tuning method name, ['LoRA', 'AdaLoRA', ...]."}
    )
    lora_rank: Optional[int] = field(default=8, metadata={"help": "The number of rank for lora-like methods."})
...
def main():
    ...
    run = wandb.init(
        ...
        name=f"{model_args.ft_method}_{lora_svd_method}_seed_{model_args.input_seed}",
    )
    ...
    """Still need it if we are using wandb?"""
    # The log saving function from DomainBed  
    os.makedirs(training_args.output_dir, exist_ok=True)
    sys.stdout = Tee(os.path.join(training_args.output_dir, 'out.txt'))
    sys.stderr = Tee(os.path.join(training_args.output_dir, 'err.txt'))
    ...
    if model_args.input_seed is not None:
        set_seed(model_args.input_seed)
    else:
    ...
    if model_args.ft_method in ["LoRA", "LoRASYM"]:
        ...
        model = load_peft_model_test(model, method=model_args.ft_method, strategy=model_args.lora_svd_method, 
                                     ...
                                     task_type_input="SEQ_CLS") # It's CAUSA_LM for llama models
    elif model_args.ft_method in ["IA3", "AdaLoRA"]:
        print("Using low-rank finetuning, ft_method =", model_args.ft_method)
        model = load_peft_model_test_2(model, method=model_args.ft_method, strategy=model_args.lora_svd_method, 
                                     model_cofig=model_args, 
                                     task_type_input="SEQ_CLS") # It's CAUSA_LM for llama models
    else:
        print("Using full-finetuing, ft_method =", model_args.ft_method)
