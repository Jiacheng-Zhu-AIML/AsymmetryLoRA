...
# from domainbed.local_lora_svd import LoraSVDModel
from .local_lora_svd_all import LoraSVDModel, LoraSVDConfig
...
PEFT_TYPE_TO_MODEL_MAPPING = {
    ...
    PeftType_local.LORA_SVD: LoraSVDModel,
}


PEFT_TYPE_TO_CONFIG_MAPPING_local: dict[str, PeftConfig] = {
    "LORA": LoraConfig,
    "LORA_SVD": LoraSVDConfig,
}
...
def get_peft_model_state_dict_local(model, state_dict=None, adapter_name="default"):
    ...
    elif config.peft_type == PeftType_local.LORA_SVD:
        """
        Need to check later?
        """
        to_return = {k: state_dict[k] for k in state_dict if "lora_" in k}
...
class PeftModel_local(PeftModel, PushToHubMixin, torch.nn.Module): # Originally using PeftModel_local(PeftModel)
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default"):
        ...
        """ 
        temp_peft_type = peft_config.peft_type # Should be "LORA_SVD"
        peft_config.peft_type = "LORA"
        super().__init__(model, peft_config, adapter_name)
        peft_config.peft_type = temp_peft_type
        """
        ...
        if not peft_config.is_prompt_learning:
            ...
            # This step is using the self-defined LoraSVDModel
    ...
    def from_pretrained_local(self, model_id: str, adapter_name: str, is_trainable: bool = False, **kwargs: Any):
        ...
        # print("type(adapters_weights) =", type(adapters_weights))
        # print("adapters_weights =", adapters_weights)
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        """
        # from peft.mapping import MODEL_TYPE_TO_PEFT_MODEL_MAPPING, PEFT_TYPE_TO_CONFIG_MAPPING # Need to change .mapping?
        """
        It seems I need a PeftConfig_local
        """
        # load the config
        if config is None:
            config = PEFT_TYPE_TO_CONFIG_MAPPING_local[
                PeftConfig._get_peft_type( # TODO: Change PeftConfig to PeftConfig_local?
                    model_id,
        ...
        if config.task_type not in MODEL_TYPE_TO_PEFT_MODEL_MAPPING_local.keys():
            model = cls(model, config, adapter_name)
        else:
            model = MODEL_TYPE_TO_PEFT_MODEL_MAPPING_local[config.task_type](model, config, adapter_name)
        model.load_adapter(model_id, adapter_name, is_trainable=is_trainable, **kwargs)
        return model


class PeftModel_local_old(PushToHubMixin, torch.nn.Module):
    """
    ...
    **Attributes**:
    ...
    def set_additional_trainable_modules(self, peft_config, adapter_name):
        ...
        print("adapter_name =", adapter_name) # adapter_name = default
        
        # print()
        
        if getattr(peft_config, "modules_to_save", None) is not None:
            print("self.modules_to_save =", self.modules_to_save)
            if self.modules_to_save is None:
                self.modules_to_save = set(peft_config.modules_to_save)
            else:
                self.modules_to_save.update(peft_config.modules_to_save)
                
            _set_trainable(self, adapter_name) # Find the model.named_modules() that have modules_to_save to requires_grad = True
...
class PeftModelForCausalLM_local_old(PeftModel_local_old):
    """
